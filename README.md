...
- 2026高難度特化練習会は8/9・8/23を1イベントとして合算し、延べ224人（124＋100）を集計。8/23の2コマ加算を保持し、継続参加率も両日を同一イベントとして人物単位で計算。会員分母は開始日8/9時点
...
公開前には、中央ダッシュボードの年度値・年度入会率・相対スコアがA〜Dで完全に整合することを必須にしています。年度値は、会員マスターの基準日付き匿名集計とも照合します。今年・前年の実体験数と体験からの入会数、年度、基準日、集計定義、合計が一致しない場合は公開を停止します。会員数の同期日だけが新しくても、古い月次保存値を年度値として公開しません。A〜D体験シートの用途は当日人数だけです。中央ダッシュボード内の不整合、未知イベント、列構成変更、検証失敗時はmainを変更しません。

「年度実入会」は入会日が年度内の件数（再入会を含む）、「年度 体験→入会率」は年度内に実体験した人のうち入会した割合です。集計対象が異なるため、実入会数を体験→入会率の分子へ置き換えません。詳細は [年度入会の集計と照合](docs/annual-admission-consistency.md) を参照してください。
...
`referral-volume-rate-v2` の匿名集計が公開元で検証できた時点で、第5指標を家庭継続力から紹介力へ切り替えます。内部キー `family` は互換性のため保持します。
...
- 人数と紹介率をそれぞれ中央値順位パーセンタイルへ変換し、人数70％＋紹介率30％で合成します。紹介率＝年度紹介pt÷基準日の運用会員数（100％を超える場合もあります）。総合配点15％を維持。全チーム紹介0人なら0点。会員数未確定・0人は算出せず公開を止めます。
- 画面には相対点と紹介ポイント実数・紹介率％、紹介体験／兄弟姉妹の内訳を表示します。定義変更・年度変更をまたぐ紹介力と総合点の差は表示しません。
